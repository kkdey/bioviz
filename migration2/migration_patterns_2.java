...
public class migration_patterns_2 extends PApplet {
 ...
 println(probMatrix[0][1]);
  ...
  for(int i=0; i< NUM_ISLANDS; i++){
    for(int j=0; j< NUM_ISLANDS; j++){
      ...
      speed[i][j] = 0.01f*probMatrix[i][j];
    ...
    for(int j=0; j < NUM_ISLANDS; j++){
      ...
      if(floor(numHumans[i][j])>0){
                    for(int k=0; k < floor(numHumans[i][j]); k++){
                           ...
                           speed_counter[flag] = speed[i][j]*k;
  ...
  for(int i=0; i < islandX.length; i++){
   ...
   for(int i=0; i < humans.length; i++){
     ...
     headXarr[i] = islandX[parent_counter[i]] + (islandX[child_counter[i]] - islandX[parent_counter[i]])*sin(angle_rad[i] * PI) 
                  + random(-speed_counter[i]*30, speed_counter[i]*30)
...
class Human{
 ...
 // float pleglength;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "migration_patterns_2" };
    if (passedArgs != null) {
